...
class RainTPL{
	...
	// -------------------------
		 ...
		 * @var string
		 */
		static $tpl_dir = "public_html/templates/";
		 ...
		 * @var string
		 */
		static $cache_dir = "resources/tpl_temp/";
	...
	const CACHE_EXPIRE_TIME = 0; // default cache expire time = hour
	...
	/*function msg( $color, $title, $msg, $cancelable ){

		try {
			$template_code = file_get_contents( '/var/www/rpi/public_html/templates/msg.html' );
			...
			$html = $this->var_replace( $template_code, $left_delimiter = '\{', $right_delimiter = '\}', $php_left_delimiter = '<?php ', $php_right_delimiter = ';?>', null, $echo = true );
			eval('?>'.$html.'<?');
	...
	function msg( $color, $title, $msg, $cancelable = true ){
		self::assign('msg_id', rand(0, 1000));
		self::assign('msg_color', ($color != '') ? 'info_'.$color : '');
		self::assign('msg_title', $title);
		self::assign('msg_msg', $msg);
		self::assign('msg_cancelable', $cancelable);
		self::draw('msg');
		unset( $this->var['msg_id'], $this->var['msg_title'], $this->var['msg_title'], $this->var['msg_msg'], $this->var['msg_cancelable'] );
	}
	
	
	
	function error( $title, $msg, $cancle = true ){
		
		self::assign('error_title', $title);
		self::assign('error_msg', $msg);
		self::draw('error');
		unset( $this->var['error_title'], $this->var['error_msg'] );
		
		$this->if_error = $cancle;
	}
