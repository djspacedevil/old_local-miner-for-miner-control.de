...
if (function_exists('fsockopen') && ini_get('allow_url_fopen') !== false)
{
	if (checkInternetConnection() === true)
	{
		if ($plugins_file = json_decode(json_encode(simplexml_load_file($config['urls']['pluginUrl'], NULL , LIBXML_NOCDATA)), true))
		{
			...
			{
				if (file_put_contents('plugin.zip', file_get_contents($config['urls']['pluginDownloadUrl'].'[Update]'.$plugins_file['plugin'][$plugin_file_id]['filename'])))
				{
					if (md5_file('plugin.zip') == $plugins_file['plugin'][$plugin_file_id]['checksumUpdate'])
					{
						...
						if (!(file_exists($plugin_folder) && is_dir($plugin_folder)))
						{
							...
							$tpl->assign('msg', 'Der Pluginordner für das Plugin konnte nicht gefunden werden.');
							$tpl->draw('single_box');
							exit();
						}
						...
						if (($zip_error = $zip->open('plugin.zip')) === true)
						{
							...
							{
								header('Location: ../../?s=plugin_search&do=information&id='.$plugins_file['plugin'][$plugin_file_id]['id'].'&statusmsg=updated');
								exit();
							}
						}
						else
